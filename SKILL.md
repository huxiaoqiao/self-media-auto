...
description: |
  ...
  **也请主动使用**：当用户提到"公众号文章"、"短视频内容"、"内容二创"、"IP 风格"、"自媒体运营"、"微信发布"、"内容改写"、"爆款分析"、"选题挖掘"、"小红书内容"、"抖音视频"等相关概念时，即使用户没有明确说要使用技能——因为这是处理此类任务最高效的方式。
---
...
- `--method browser`：使用高级 CDP 驱动的浏览器自动化，支持**原生文件注入**、**封面 2.35:1 比例自动切换**以及**远程扫码登录**（二维码生成在 `assets/login_qr.png`）。
...
├── assets/                     # 静态资源
│   └── login_qr.png            # 微信扫码登录二维码 (关键：需推送给用户)
└── SKILL.md                    # 本文件
```
