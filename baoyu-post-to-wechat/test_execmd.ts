  ...
  const result = await cdp.send('Runtime.evaluate', {
    expression: `
      (function() {
        ...
        // 使用 execCommand 插入 HTML
        const success = document.execCommand('insertHTML', false, ${JSON.stringify(content)});
        
        return 'execCommand result: ' + success + ', content: ' + editor.innerHTML.length;
      })()
    `,
    returnByValue: true,
  }, { sessionId });
  
  console.log('插入结果:', result.result.value);
