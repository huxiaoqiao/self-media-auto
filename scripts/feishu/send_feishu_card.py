...
def upload_image(token: str, image_path: str) -> str:
    ...
    if image_key:
        ...
        print(f"  ✗ 图片上传失败: {data.get('msg', 'unknown error')}")
    return image_key
...
def _format_topic_analysis(t: dict) -> str:
    ...
    if t.get('score') and int(t.get('score', 0)) > 10000:
        return "爆款选题，热度极高"
    if '震惊' in title or '紧急' in title or 'Source' in title:
        return "震惊体标题，吸引力强但需甄别内容质量"
    if '教程' in title or 'Open' in title:
        return "教程类内容，实用性强受众广泛"
    if 'AI' in title and ('重磅' in title or '震撼' in title or '革命' in title):
        return "AI领域重磅话题，传播性强"
    return "内容有价值，值得关注"
...
def build_topic_list_card(topics: list, industry: str = "") -> dict:
    """选题列表卡片 - 展示多个选题"""
    elements = []
    elements.append({"tag": "markdown", "content": f"**🔥 {industry}赛道 · 为您精选 TOP {len(topics)}**"})
    ...
    elements.append({
        "tag": "action",
        "actions": [
            {"tag": "button", "text": {"tag": "plain_text", "content": "📋 换一批（查看更多爆款选题）"}, "type": "default", "value": {"action": "next"}},
            {"tag": "button", "text": {"tag": "plain_text", "content": "⚙️ 初始化（重置 IP 名称/行业赛道）"}, "type": "default", "value": {"action": "init"}}
        ]
    })
    elements.append({
        "tag": "note",
...
def build_url_preview_card(title: str, author: str, source: str, summary: str, url: str, content_type: str, extra_info: str = "") -> dict:
    ...
    return {
        ...
        "elements": [
            ...
            {"tag": "action", "actions": [
                {"tag": "button", "text": {"tag": "plain_text", "content": "✍️ 开始改写"}, "type": "primary", "value": {"action": "rewrite_url", "type": content_type}},
                {"tag": "button", "text": {"tag": "plain_text", "content": "❌ 算了"}, "type": "default", "value": {"action": "cancel_url"}}
            ]},
...
def build_rewrite_card(title: str, insight: str, topic_id: str) -> dict:
    ...
    return {
        ...
        "elements": [
            ...
            {"tag": "action", "actions": [
                {"tag": "button", "text": {"tag": "plain_text", "content": "✍️ 开始改写"}, "type": "primary", "value": {"action": "rewrite", "id": topic_id}},
                {"tag": "button", "text": {"tag": "plain_text", "content": "🔄 换一个"}, "type": "default", "value": {"action": "next_new"}}
            ]}
        ]
    }
...
def build_review_card(image_key: str, title: str, content: str, tags: str, review_id: str, template: str = "blue", header_title: str = None) -> dict:
    ...
    elements.append({
        "tag": "note",
        "elements": [{"tag": "plain_text", "content": f"生成时间: {now_str}"}]
    })
    elements.append({
        "tag": "action",
        "actions": [
            {"tag": "button", "text": {"tag": "plain_text", "content": "✅ 通过"}, "type": "primary", "value": {"action": "approve", "id": review_id}},
            {"tag": "button", "text": {"tag": "plain_text", "content": "✏️ 修改"}, "type": "default", "value": {"action": "modify", "id": review_id}},
            {"tag": "button", "text": {"tag": "plain_text", "content": "🔄 重写"}, "type": "default", "value": {"action": "rewrite", "id": review_id}}
        ]
    })
    ...
    return {
        ...
        "elements": [
            ...
            {"tag": "action", "actions": [
                ...
                {"tag": "button", "text": {"tag": "plain_text", "content": "✏️ 调整内容"}, "type": "default", "value": {"action": "edit", "id": topic_id}}
            ]}
        ]
    }
...
def build_final_card(image_key: str, title: str, content: str, tags: str, review_id: str, article_image_keys=None, deai_notes: str = None) -> dict:
    ...
    if article_image_keys:
        elements.append({"tag": "hr"})
        elements.append({"tag": "note", "elements": [{"tag": "plain_text", "content": f"📎 插图 {len(article_image_keys)} 张，见下方或飞书文档"}]})
    ...
    if deai_notes:
        ...
        elements.append({
            "tag": "div",
            "text": {"tag": "lark_md", "content": f"**ℹ️ Natural Chinese Protocol 与 AI 处理说明：**\n\n{deai_notes}"}
        })
    ...
    elements.append({
        "tag": "action",
        "actions": [
            {"tag": "button", "text": {"tag": "plain_text", "content": "📤 发布到公众号"}, "type": "primary", "value": {"action": "post", "id": review_id}},
            {"tag": "button", "text": {"tag": "plain_text", "content": "📋 复制文案"}, "type": "default", "value": {"action": "copy", "id": review_id}}
        ]
    })

    return {
        "config": {"wide_screen_mode": True},
        "header": {"template": "green", "title": {"tag": "plain_text", "content": "🎉 最终稿确认"}},
        "elements": elements
    }
...
def main():
    ...
    if card_type == "topic_list":
        ...
        if extra_images:
            ...
            for idx, img_path in enumerate(extra_images):
                if os.path.exists(img_path):
                    ...
                    if img_key:
                        caption = f"插图 {idx+1}" if idx > 0 else "封面图"
                        # 使用消息接口发送图片
                        ...
                        try:
                            with urllib.request.urlopen(img_req, timeout=10) as resp:
                                ...
                                if result.get('code') == 0:
                                    print(f"  ✓ 图片 {idx+1} 发送成功")
                                else:
                                    print(f"  ✗ 图片 {idx+1} 发送失败: {result.get('msg')}")
                        except Exception as e:
                            print(f"  ✗ 图片 {idx+1} 发送异常: {e}")
        ...
        print("支持的类型: topic_list | rewrite | review | archive | final")
        sys.exit(1)
