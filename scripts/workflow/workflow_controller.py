...
# ==================== 添加项目根目录和模块目录到 sys.path ====================
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
_MODULES_DIR = os.path.join(_PROJECT_ROOT, "scripts", "modules")
...
class SelfMediaController:
    ...
    def run_from_article(self, url_or_text):
        ...
        logger.info("[FROM_ARTICLE] Saving .env configuration")
    ...
    def run_next_discovery(self):
        ...
        for idx, c in enumerate(page_items, start_idx + 1):
            ...
            logger.info("[DISCOVERY] Using cached last_id: %s", state.get('cimi_last_id', 'N/A'))
            print(f"👤 {author} | 👁️ 阅读: {reads} | 👍 赞: {likes} | 🔥 热度：{score}")
            logger.info("[DISCOVERY] Starting from ID: %s", state.get('cimi_last_id', 'N/A'))
        print("===================================")
    ...
    def _extract_article_content(self, article_url, selected=None):
        ...
        if source in ["公众号", "微信", "微信公众号(次幂)", "自定义"] or str(article_url).startswith("http"):
            # 1. url-reader 引擎
            try:
                reader_path = os.path.join(_PROJECT_ROOT, 'scripts', 'modules', 'url-reader-0.1.1', 'scripts')
            ...
            try:
                dj = os.path.join(_PROJECT_ROOT, "scripts", "modules", "douyin-download-1.2.0", "douyin.js")
    ...
    def _extract_video_content(self, url):
        ...
        if "douyin.com" in url:
            ...
            douyin_js_path = os.path.join(_PROJECT_ROOT, "scripts", "modules", "douyin-download-1.2.0", "douyin.js")
...
微信号：{wechat_id}"""
    ...
    DEAI_PROMPT_TEMPLATE = """# Role：文章润色专家
...
- 引号和加粗要谨慎使用，只在真正需要特别强调或引用时使用，每段最多1-2处
...
"""
    ...
    def de_ai_style(self, content: str, api_key: str, api_base: str, model_id: str):
        ...
        # 支持外部配置提示词
        pm_path = os.path.join(self.workspace, "references", "prompts")
        deai_file = os.path.join(pm_path, "deai-prompt.md")
        if os.path.exists(deai_file):
            with open(deai_file, 'r', encoding='utf-8') as f:
                deai_prompt = f.read()
        elif os.path.exists(os.path.join(self.workspace, "prompts_manager.json")):
            with open(os.path.join(self.workspace, "prompts_manager.json"), 'r', encoding='utf-8') as f:
                conf = json.load(f)
            ...
            if is_html:
                ...
                if article_images:
                    for i, img_info in enumerate(article_images):
                        ...
                        tag = f'<img src="WECHATIMGPH_{i + 1}" data-local-path="{_os.path.abspath(img_path)}" style="max-width: 100%; height: auto; display: block; margin: 20px auto;" />'
