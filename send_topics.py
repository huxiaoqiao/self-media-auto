...
# Top 5 topics from last_candidates
topics = [
    {
        "id": "1",
...
card_script = os.path.join(script_dir, "send_feishu_card.py")
...
for topic in topics:
    ...
    result = subprocess.run(cmd, capture_output=True, text=True, env={'PYTHONIOENCODING': 'utf-8'})
    print(result.stdout)
    if result.stderr:
        print(result.stderr)

print("\nAll topic cards sent!")
