    ...
    def run_repurpose(self, topic_id_or_cmd):
        ...
        try:
            with open(abs_script_path, "w", encoding='utf-8') as f:
                f.write(video_script if video_script else "（脚本生成失败）")
            ...
            if os.path.exists(abs_article_path):
                print(f"✅ [物理存档成功]: {abs_article_path}", flush=True)
                logger.info(f"✅ [物理存档成功]: {abs_article_path}")
        except Exception as e:
...
��规范\n"
        ...
        # 核心修复：确保 drafts 文件夹在正确的技能根目录下
        skill_root = os.path.dirname(os.path.abspath(__file__))
        drafts_root = os.path.join(skill_root, 'drafts')
        ...
        def clean_headers(text):
            if not text: return ""
            # 移除所有可能的块标识符
            pats = [r"##?\s*第[一二12]部分.*?\n", r"【爆款短视频脚本】", r"【深度长文.*?】", r"##?\s*脚本内容", r"##?\s*文章内容", r"---"]
            for p in pats: text = re.sub(p, "", text, flags=re.I)
            return text.strip()

        # 尝试通过标记切分
        blocks = re.split(r"(##?\s*第[二2]部分|【深度长文|文章正文|文章内容|---)", final_content, flags=re.I)
        if len(blocks) >= 2:
            video_script = clean_headers(blocks[0])
            article_content = clean_headers("".join(blocks[1:]))
        else:
            # 兜底：寻找标题
            ts = title_pattern.search(final_content)
            if ts:
                video_script = clean_headers(final_content[:ts.start()])
                article_content = clean_headers(final_content[ts.start():])
            else:
